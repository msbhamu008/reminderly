...
export async function POST(request: Request) {
  try {
    ...
    // Convert jobType from underscore to hyphen format
    ...
    const response = await fetch(cronJobUrl, {
      ...
      body: JSON.stringify({
        url: `https://reminderly-one.vercel.app/api/cron/${formattedJobType}`,
    ...
    return NextResponse.json({ success: true, message: `Job ${formattedJobType} scheduled successfully` });
  } catch (error) {
